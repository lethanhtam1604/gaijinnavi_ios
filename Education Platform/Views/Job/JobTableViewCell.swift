...
class JobTableViewCell: UITableViewCell {
    ...
    func commonInit() {
        ...
        logoJobButton.setTitleColor(UIColor.white, for: .normal)
        logoJobButton.titleLabel?.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.headline)
        logoJobButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 12)
        logoJobButton.layer.cornerRadius = 1
        logoJobButton.clipsToBounds = true
        logoJobButton.contentMode = .scaleAspectFill
        logoJobButton.backgroundColor = UIColor.white
        ...
        highlightBtn.setTitleColor(UIColor.white, for: .normal)
        highlightBtn.titleLabel?.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.headline)
        highlightBtn.titleLabel?.font = UIFont(name: "OpenSans", size: 12)
        highlightBtn.layer.cornerRadius = 2
        highlightBtn.clipsToBounds = true
        highlightBtn.backgroundColor = UIColor.red
    ...
    override func updateConstraints() {
        ...
        if !constraintAdded {
            ...
            companyLabel.autoPinEdge(.left, to: .right, of: logoJobButton, withOffset: 10)
            companyLabel.autoPinEdge(.right, to: .left, of: highlightBtn, withOffset: 10)
            
            highlightBtn.autoPinEdge(toSuperviewEdge: .top, withInset: 10)
            highlightBtn.autoPinEdge(toSuperviewEdge: .right, withInset: 10 + margin)
            highlightBtn.autoSetDimension(.height, toSize: 23)
            highlightBtn.autoSetDimension(.width, toSize: 100)
    ...
    func bindingData(jobListResult: JobListResult) {
        DispatchQueue.main.async {
            ...
            self.highlightBtn.setTitle("highlighted".localized(), for: .normal)
        ...
        if jobListResult.PostData.Highlight {
            self.highlightBtn.isHidden = false
        }
        else {
            self.highlightBtn.isHidden = true
        }
    }
}
