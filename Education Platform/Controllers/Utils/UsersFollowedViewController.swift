//
//  UsersFollowedViewController.swift
//  Education Platform
//
...
class UsersFollowedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, FollowServiceDelegate {
    
    @IBOutlet weak var tableView: UITableView!
    let followCellReuseIdentifier = "FollowCellReuseIdentifier"
    
    var user: UserResult!
    var usersFollowed = [UserResult]()
...
//        }
    ...
    func setLanguageRuntime(){
        self.title = user.DisplayName + " " +  "is_followed_by".localized()
    }
    ...
    func refresh() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now()) {
            self.currentPage = 1
            self.loadType = true
            FollowService.getFollowedList(userId: self.user.Id, followServiceDelegate: self)
        }
    }
    ...
    func title(forEmptyDataSet scrollView: UIScrollView!) -> NSAttributedString! {
        let text = "no_follower_entry".localized()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let user = usersFollowed[indexPath.row]
    ...
    func getFollowedListFinished(success: Bool, message: String, result: [UserResult]) {
        if success {
            if loadType {
                self.usersFollowed = [UserResult]()
            }
            self.usersFollowed.append(contentsOf: result)
